...
object Api {
  ...
  fun pollSignal(sp: SharedPreferences): String? {
    val url = sp.getString("url", "") ?: ""
    val key = sp.getString("key", "") ?: ""
    if (url.isEmpty() || key.isEmpty()) return null
    return try {
      val c = URL(url + "/rest/v1/control?id=eq.1&select=poll_now").openConnection() as HttpURLConnection
      c.requestMethod = "GET"
      ...
      c.disconnect()
      val a = resp.indexOf('{'); val b = resp.lastIndexOf('}')
      if (a < 0 || b <= a) return null
      val o = JSONObject(resp.substring(a, b + 1))
      if (o.has("poll_now") && !o.isNull("poll_now")) o.optString("poll_now") else null
  ...
  private fun startDrive() {
    ...
    worker = Thread {
      ...
      try {
        ...
        var count = 0
        var netfail = 0
        var lastPoll: String? = Api.pollSignal(sp)
        while (driving) {
          // 토큰/차량ID 확보 — 네트워크(DNS) 실패해도 포기하지 말고 계속 재시도
          if (at == null) {
            try {
              at = Api.accessToken(sp)
              vid = Api.vehicleId(at)
              netfail = 0
              update("주행 기록 중... (" + count + "개 저장)")
            } catch (e: Exception) {
              netfail++
              update("네트워크 재연결 대기... (" + netfail + ")")
              var w = 0
              while (driving && w < 15) { Thread.sleep(1000); w++ }
              continue
            }
          }
          try {
            ...
            val m = e.message ?: ""
            if (m.contains("401") || m.contains("403")) { at = null; vid = null }
          }
          ...
          while (driving && n < iv) {
            Thread.sleep(1000); n++
            if (n % 3 == 0 && at != null) {
              val pn = Api.pollSignal(sp)
              if (pn != null) {
                if (lastPoll == null) lastPoll = pn
                else if (pn != lastPoll) { lastPoll = pn; break }
              }
            }
          }
        }
